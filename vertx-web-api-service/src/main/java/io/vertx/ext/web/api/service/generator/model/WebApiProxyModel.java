...
public class WebApiProxyModel extends ProxyModel {
  ...
  protected MethodInfo createMethodInfo(Set<ClassTypeInfo> ownerTypes, String methodName, String comment, Doc doc, TypeInfo returnType, Text returnDescription, boolean isFluent, boolean isCacheReturn, List<ParamInfo> mParams, ExecutableElement methodElt, boolean isStatic, boolean isDefault, ArrayList<TypeParamInfo.Method> typeParams, TypeElement declaringElt, boolean methodDeprecated, Text deprecatedDesc) {
    ...
    if (!isStatic && !baseInfo.isProxyClose()) {
      if (mParams.size() < 2) {
        throw new GenException(this.modelElt, "Method should have second to last parameter of type io.vertx.ext.web.api.service.ServiceRequest and last parameter of type Handler<AsyncResult<io.vertx.ext.web.api.service.ServiceResponse>>");
      }
      ParamInfo shouldBeRequestContextParam = mParams.get(mParams.size() - 2);
      if (shouldBeRequestContextParam == null || !shouldBeRequestContextParam.getType().getName().equals(ServiceRequest.class.getName())) {
        throw new GenException(this.modelElt, "Method " + methodName + "should have the second to last parameter with type io.vertx.ext.web.api.service.ServiceRequest");
      }
      ...
      if (baseInfo.getKind() != MethodKind.HANDLER || shouldBeHandler == null) {
        ...
        if (!ServiceResponse.class.getName().equals(shouldBeOperationResult.getName()))
          throw new GenException(this.modelElt, "Method " + methodName + "should last parameter should be an handler of type Handler<AsyncResult<io.vertx.ext.web.api.service.ServiceResponse>>");
      }
