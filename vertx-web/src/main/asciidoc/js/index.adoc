...
A `link:../../jsdoc/router-Router.html[Router]` is one of the core concepts of Vert.x-Web. It's an object which maintains zero or more
...
You set a blocking handler on a route with `link:../../jsdoc/route-Route.html#blockingHandler[blockingHandler]`.
Here's an example:
...
false using `link:../../jsdoc/route-Route.html#blockingHandler[blockingHandler]`.
...
var route = router.get("/localized").handler(function (rc) {
  ...
  Array.prototype.forEach.call(rc.acceptableLocales(), function(locale) {
    return
  });
...
The main method `link:../../jsdoc/routing_context-RoutingContext.html#acceptableLocales[acceptableLocales]` will return the ordered list of locales the
user understands, if you're only interested in the user prefered locale then the helper:
`link:../../jsdoc/routing_context-RoutingContext.html#preferredLocale[preferredLocale]` will return the 1st element of the list or `null` if no
...
Vert.x-Web has cookies support using the `link:../../jsdoc/cookie_handler-CookieHandler.html[CookieHandler]`.
...
In your handlers you can access the session instance with `link:../../jsdoc/routing_context-RoutingContext.html#session[session]`.
...
a clustered session store they can be any basic type, or `link:../../jsdoc/buffer-Buffer.html[Buffer]`, `JsonObject`,
...
To create an auth handler you need an instance of `link:../../jsdoc/auth_provider-AuthProvider.html[AuthProvider]`. Auth provider is
...
If the auth handler has successfully authenticated and authorised the user it will inject a `link:../../jsdoc/user-User.html[User]`
object into the `link:../../jsdoc/routing_context-RoutingContext.html[RoutingContext]` so it's available in your handlers with:
`link:../../jsdoc/routing_context-RoutingContext.html#user[user]`.
...
If you want to cause the user to be logged out you can call `link:../../jsdoc/routing_context-RoutingContext.html#clearUser[clearUser]`
...
When the basic auth handler receives this information, it calls the configured `link:../../jsdoc/auth_provider-AuthProvider.html[AuthProvider]`
...
Vert.x-Web includes a handler `link:../../jsdoc/cors_handler-CorsHandler.html[CorsHandler]` that handles the CORS protocol for you.
...
Vert.x-Web includes a handler `link:../../jsdoc/logger_handler-LoggerHandler.html[LoggerHandler]` that you can use to log HTTP requests.
...
Vert.x-Web includes the handler `link:../../jsdoc/favicon_handler-FaviconHandler.html[FaviconHandler]` especially for serving favicons.
...
The event enables you to retrieve the type using `link:../../jsdoc/bridge_event-BridgeEvent.html#type[type]` and
inspect the raw message of the event using `link:../../jsdoc/bridge_event-BridgeEvent.html#rawMessage[rawMessage]`.
...
The event is also an instance of `link:../../jsdoc/future-Future.html[Future]`. When you are finished handling the event you can
...
Vert.x-Web includes a handler `link:../../jsdoc/csrf_handler-CSRFHandler.html[CSRFHandler]` that you can use to prevent cross site
